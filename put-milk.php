...
$sql = 'UPDATE milk SET name=?, brand=?, type=?, weight=?, price=?, nutritionalIngredients=?, benefit=?, image=? WHERE id=?';

try {
    $sql1 = 'SELECT * FROM milk WHERE id=?';
    $stmt1 = $dbCon->prepare($sql1);
    $stmt1->execute(array($id));
    $result = $stmt1->fetch(PDO::FETCH_ASSOC);

    // Xử lý upload hình ảnh và nhận đường dẫn của nó
    $image = (isset($_FILES['image']) && !empty($_FILES['image']['name'])) ? uploadImage($_FILES['image']) : $result['image'];

    $stmt = $dbCon->prepare($sql);
    $stmt->execute(array($name, $brand, $type, $weight, $price, $nutritionalIngredients, $benefit, $image, $id));

    echo json_encode(array('status' => true, 'data' => $id, 'message' => 'Cập nhật dữ liệu trong bảng milk thành công'));
